...
def runLogisticLinear(name):
    ...
    #train the logistic linear model
    w = np.array([[0,0,0]])
    ...
    with open('Q1_ROC_'+name+'_linear.npy', 'wb') as f1:
...
def plotROC_logisticLinear():
    #load ROC data from part A,B
    ROCtruePdf = np.load(open('Q1_ROC1.npy', 'rb'))
    ROC20 = np.load(open('Q1_ROC_D_train20_linear.npy', 'rb'))
    ROC200 = np.load(open('Q1_ROC_D_train200_linear.npy', 'rb'))
    ROC2000 = np.load(open('Q1_ROC_D_train2000_linear.npy', 'rb'))

    # plot the ROC curve
    fig,ax = plt.subplots()
    l0,=ax.plot(ROCtruePdf[:,0], ROCtruePdf[:,1], color='tab:green',zorder=0,label='True PDF used')
    l1,=ax.plot(ROC20[:,0], ROC20[:,1], color='tab:pink',zorder=1,label='Logistic Linear trained on 20 Samples')
    l2,=ax.plot(ROC200[:,0], ROC200[:,1], color='tab:orange',zorder=2,label='Logistic Linear trained on 200 Samples')
    l3,=ax.plot(ROC2000[:,0], ROC2000[:,1], color='tab:blue',linestyle='dashed',zorder=3,label='Logistic Linear trained on 2000 Samples')
    ...
    ax.set_title('Question 1 Part 2 ROC comparisons with Logistic Linear models')
...
runLogisticLinear('D_train20')
runLogisticLinear('D_train200')
runLogisticLinear('D_train2000')
plotROC_logisticLinear()